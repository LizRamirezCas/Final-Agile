document.addEventListener('DOMContentLoaded', () => {
    ...
    // Elementos nuevos para navegación
    ...
    // ESTADO DE NAVEGACIÓN (Null = Vista normal "Hoy", Fecha = Vista de ese mes específico)
    ...
    supabase.auth.onAuthStateChange((event, session) => {
        if (event === 'SIGNED_IN' && session) startSession(session.user);
    ...
    const startSession = async (user) => {
        ...
        if (Notification.permission !== "granted" && Notification.permission !== "denied") await Notification.requestPermission();
        checkDebtsDueToday(user.id);
        if (checkInterval) clearInterval(checkInterval);
        checkInterval = setInterval(() => checkDebtsDueToday(user.id), 3600000); 
    };

    const checkDebtsDueToday = async (userId) => {
        const hoyString = getLocalISOString();
        const { data: deudasHoy } = await supabase.from('debts').select('*').eq('user_id', userId).lte('fecha', hoyString).eq('pagado', false);
    ...
    // --------------------------------------------------------------------------
    // 6. RENDERIZADO (CON NAVEGACIÓN DE MESES)
    // --------------------------------------------------------------------------
    const renderAllDebts = async () => {
        ...
        // Determinar qué mostrar según el modo de vista (Hoy vs Mes Futuro)
        let filtroDeudas;
        
        if (viewingDate === null) {
            // MODO POR DEFECTO: Mes Actual + Vencidas
            pendientesTitle.textContent = "Deudas Pendientes (Mes Actual y Vencidas)";
            btnResetView.classList.add('hidden');
            ...
            // MODO NAVEGACIÓN: Mes Específico
            const nombreMes = viewingDate.toLocaleString('es-ES', { month: 'long', year: 'numeric' });
            pendientesTitle.textContent = `Deudas de ${nombreMes.toUpperCase()}`;
            btnResetView.classList.remove('hidden');

            filtroDeudas = (d) => {
                if (d.pagado) return false;
                const [anio, mes, dia] = d.fecha.split('-').map(Number);
                // Filtrar solo las de ese mes y año exacto
                return (mes - 1) === viewingDate.getMonth() && anio === viewingDate.getFullYear();
            };
        }

        const pendientes = deudas.filter(filtroDeudas);
        ...
        for (const key in groupedDebts) {
            ...
            groupedDebts[key].forEach(deuda => {
                ...
                if (deuda.fecha === hoyString) {
                    statusClass = 'alerta-hoy'; statusLabel = '<span class="texto-amarillo">(VENCE HOY)</span>';
                } else if (deuda.fecha < hoyString) {
                    statusClass = 'vencida'; statusLabel = '<span class="texto-rojo">(DEUDA VENCIDA)</span>';
                } else if (fechaDeuda <= finDeSemana && viewingDate === null) {
                    statusClass = 'alerta-semana'; statusLabel = '<span class="texto-amarillo">(ESTA SEMANA)</span>';
                } 

                // --- LÓGICA DE FLECHAS DE NAVEGACIÓN ---
                let nextArrowHtml = '';
                let prevArrowHtml = '';
                
                // Detectar formato "Nombre (Cuota X/Y)"
                const match = deuda.nombre.match(/^(.*) \(Cuota (\d+)\/(\d+)\)$/);
                if (match) {
                    const baseName = match[1];
                    const currentQuota = parseInt(match[2]);
                    const totalQuota = parseInt(match[3]);

                    // Buscar siguiente cuota
                    if (currentQuota < totalQuota) {
                        const nextName = `${baseName} (Cuota ${currentQuota + 1}/${totalQuota})`;
                        // Buscar en TODAS las deudas, no solo las filtradas
                        const nextDebt = deudas.find(d => d.nombre === nextName);
                        if (nextDebt) {
                            nextArrowHtml = `<button class="action-btn btn-nav" onclick="jumpToDate('${nextDebt.fecha}')" title="Ir a la siguiente cuota (${nextDebt.fecha})">➡️</button>`;
                        }
                    }
                    
                    // Buscar cuota anterior (opcional, para regresar)
                    if (currentQuota > 1) {
                         const prevName = `${baseName} (Cuota ${currentQuota - 1}/${totalQuota})`;
                         const prevDebt = deudas.find(d => d.nombre === prevName);
                         if (prevDebt) {
                             prevArrowHtml = `<button class="action-btn btn-nav" onclick="jumpToDate('${prevDebt.fecha}')" title="Ir a cuota anterior">⬅️</button>`;
                         }
                    }
                }
                ...
                el.innerHTML = `
                    ...
                    <div class="deuda-actions" style="display: flex; gap: 5px; flex-wrap: wrap; align-items: center;">
                        ${prevArrowHtml}
        ...
        pagadas.forEach(deuda => {
            ...
            const linkComprobante = deuda.comprobante_url ? `<br><a href="${deuda.comprobante_url}" target="_blank" style="color: #3498db; text-decoration: underline; font-size: 0.9em;">Ver Comprobante</a>` : '';
    ...
    // --- NUEVAS FUNCIONES DE NAVEGACIÓN GLOBAL ---
    window.jumpToDate = (dateString) => {
        const [year, month, day] = dateString.split('-').map(Number);
        // Establecer fecha de vista
        viewingDate = new Date(year, month - 1, day);
        renderAllDebts();
    };
        ...
        comprobanteInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                if (!validTypes.includes(file.type)) {
                    ...
                    this.value = ''; previewContainer.classList.add('hidden'); return;
                }
    ...
    if(confirmUploadBtn) {
        confirmUploadBtn.addEventListener('click', async () => {
            ...
            if (!file) { alert("⚠️ Por favor selecciona una imagen del comprobante."); return; }
            ...
            try {
                // OCR
                const palabrasClave = ['total', 'importe', 'monto', 's/', 'soles', 'fecha', 'hora', 'destino', 'yape', 'plin', 'transferencia', 'pago', 'exitoso', 'constancia', 'banco', 'operación', 'bcp', 'bbva', 'interbank', 'scotiabank', 'recibo', 'ticket', 'confirmación'];
                ...
                const { data: storageData, error: storageError } = await supabase.storage.from('comprobantes').upload(fileName, file, { cacheControl: '3600', upsert: false });
    ...
    const executePayment = async (metodo, comprobanteUrl = null) => {
        const updateData = { pagado: true, fecha_pago_real: new Date(), metodo_pago: metodo, comprobante_url: comprobanteUrl };
        const { error } = await supabase.from('debts').update(updateData).eq('id', currentDebtIdToPay);
        if (error) {
            console.error(error); alert('Error al procesar pago: ' + error.message);
            if(confirmUploadBtn) { confirmUploadBtn.textContent = "Confirmar y Guardar"; confirmUploadBtn.disabled = false; }
        } else {
            paymentModal.classList.add('hidden'); currentDebtIdToPay = null; renderAllDebts();
            if(confirmUploadBtn) { confirmUploadBtn.textContent = "Confirmar y Guardar"; confirmUploadBtn.disabled = false; }
    ...
    formDeuda.addEventListener('submit', async (e) => {
        ...
        if (meses <= 1) {
            ...
            for (let i = 0; i < meses; i++) {
                deudas.push({
                    ...
                    monto: cuota.toFixed(2), fecha: addMonthsToDate(fecha, i),
                    tasa_interes: tasa, numero_meses: meses
                });
            }
        }
        ...
        if (error) alert('Error: ' + error.message);
        else { alert("Deuda guardada correctamente."); formDeuda.reset(); renderAllDebts(); showSection(menuSection, "Panel Principal"); }
    });
    ...
    editForm.addEventListener('submit', async (e) => { e.preventDefault(); editModal.classList.add('hidden'); });
    checkSessionOnLoad();
});
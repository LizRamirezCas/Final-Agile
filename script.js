document.addEventListener('DOMContentLoaded', () => {
    ...
    // ======================================================================
    // >>> INICIO BLOQUE 1: NUEVAS REFERENCIAS Y VARIABLES GLOBALES <<<
    // ======================================================================

    // Referencias del Menú Desplegable (Nuevas)
    const viewSelectorBtn = document.getElementById('view-selector-btn');
    const viewDropdown = document.getElementById('view-dropdown');
    const currentViewTitle = document.getElementById('current-view-title');
    const arrowIcon = document.querySelector('.arrow-icon');
    
    // ESTADO DEL FILTRO (Por defecto: pendientes)
    let currentFilterMode = 'pendientes'; 
    ...
    // ======================================================================
    // >>> INICIO BLOQUE 2: LÓGICA DEL MENÚ DESPLEGABLE <<<
    // ======================================================================
    ...
    document.addEventListener('click', (e) => {
        if (viewDropdown && !viewDropdown.contains(e.target) && !viewSelectorBtn.contains(e.target)) {
            viewDropdown.classList.add('hidden-menu');
            if(arrowIcon) arrowIcon.classList.remove('rotate');
        }
    });

    // Función GLOBAL para cambiar filtro
    window.changeViewFilter = (mode) => {
        ...
        // Llamar a la función de renderizado con el nuevo filtro
        renderAllDebts(); 
    };
    ...
    // MODIFICACIÓN: Al ir a la vista de deudas, se usa el nuevo changeViewFilter
    goToViewBtn.addEventListener('click', () => { 
        window.changeViewFilter('pendientes'); 
        showSection(viewDebtsSection, "Deudas y Pagos"); 
    });
    ...
    // ======================================================================
    // >>> INICIO BLOQUE 3: RENDERIZADO UNIFICADO (REEMPLAZAR FUNCIÓN) <<<
    // ======================================================================
    ...
    // ======================================================================
    // >>> INICIO BLOQUE 4: LÓGICA DE PAGOS (REEMPLAZAR Y AGREGAR NUEVAS FUNCIONES) <<<
    // ======================================================================
    
    // --- NUEVA FUNCIÓN: Obtener detalles de una deuda por ID ---
    const getCurrentDebtInfo = async (debtId) => {
        ...
        if (error) {
            console.error("Error al obtener información de la deuda:", error.message);
            return null;
        }
        return data;
    };
    ...
    // --- FUNCIÓN DE PAGO MODIFICADA CON VALIDACIÓN DE ORDEN ---
    window.processPayment = async (metodo) => {
        if (!currentDebtIdToPay) return;

        const currentDebt = await getCurrentDebtInfo(currentDebtIdToPay);
        if (!currentDebt) {
            alert("Error: No se pudo cargar la información de la deuda.");
            return;
        }

        // 1. Verificar si la deuda pertenece a un plan de cuotas (Cuota X/Y)
        const cuotaMatch = currentDebt.nombre.match(/\(Cuota (\d+)\/(\d+)\)/);

        if (cuotaMatch) {
            const cuotaActual = parseInt(cuotaMatch[1]);
            const nombreBase = currentDebt.nombre.replace(cuotaMatch[0], '').trim();

            // Si es la segunda cuota o superior (Cuota >= 2)
            if (cuotaActual > 1) {
                const cuotaAnterior = cuotaActual - 1;
                
                // 2. Construir el nombre de la cuota anterior a buscar
                const nombreCuotaAnterior = `${nombreBase} (Cuota ${cuotaAnterior}/${cuotaMatch[2]})`;

                // 3. Buscar la cuota anterior en la base de datos para verificar su estado
                const { data: previousDebt, error: prevError } = await supabase
                    .from('debts')
                    .select('pagado')
                    .eq('user_id', currentDebt.user_id)
                    .eq('nombre', nombreCuotaAnterior)
                    .single();

                if (prevError && prevError.code !== 'PGRST116') {
                    // Si el error es distinto a 'No rows found', lo reportamos pero no bloqueamos (fallback)
                    console.error("Error al verificar cuota anterior:", prevError);
                }

                // 4. Si la cuota anterior existe y NO está pagada, bloqueamos el pago.
                // Usamos previousDebt?.pagado === false para manejar el caso donde se encuentra la fila pero está pendiente
                if (previousDebt && previousDebt.pagado === false) {
                    alert(`❌ Debes pagar primero la Cuota ${cuotaAnterior}/${cuotaMatch[2]} antes de proceder con esta cuota.`);
                    return; // Bloquea el proceso de pago
                }
            }
        }
        
        // 5. Si no hay bloqueo, procedemos con el pago:
        ...
        if (!confirm(`¿Confirmar pago con ${metodo} para la deuda ${currentDebt.nombre}?`)) return;
    ...
    if(confirmUploadBtn) {
        confirmUploadBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // Usar Tesseract.recognize que es asíncrono
                ...
                if (!esComprobanteValido) {
                    // CORRECCIÓN DE ERROR DE CARACTERES EN EL ALERT
